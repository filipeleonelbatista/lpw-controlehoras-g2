...
@manager.command
def adduser(matricula, fullusername, username, email, admin=True):
    ...
    db.create_all()
    user = User(matricula=matricula, fullusername=fullusername, username=username, email=email, password=password, is_admin=admin)
    db.session.add(user)
    db.session.commit()
